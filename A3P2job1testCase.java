...
//test case for job type 1 (A3P2Q1 Job 1)
public class A3P2job1testCase{ //client
    public static void main(String[] args) throws IOException, IllegalRawDataException {
        ...
        String[] testIps[3] = {"www.google.com", "www.facebook.com", "www.twitter.com"};
        ...
        while (i < 3) {
            ...
            while (j < 0) {
                try {
                    ...
                    System.out.println("Warning: Doing this against a valid IP address may count as an illegal DDoS attack");
                    System.out.println("Please use dummy IP 0.0.0.0");
                    jobType = "1";//job type is 1 for this test case
                    pr.println(jobType);
                    pr.flush();
                    //get ip address from user
                    System.out.println("Enter an IP Address/host name:");
                    ip = testIps[j];//use test case sample data
                    pr.println(ip);
                    pr.flush();
